...
int hashTable::setPointer(const std::string &key, void *pv){
	...
	if( this->contains(key) ) {
		//win!
		//sets pv at hashed position in data vector ptr
		(*this->data)[findPos(key)].pv = pv;
		return 0;
	}
...
bool hashTable::remove(const std::string &key){
	...
	if( this->contains(key) ) {
		//win!
		//lazy deletes the bastard
		(*this->data)[findPos(key)].isDeleted = true;
		return true;
	}
...
int hashTable::findPos(const std::string &key){
    ...
    while(1){
		...
		//return if deleted -- keep searching
        if(((*this->data)[hashedKey+i].isDeleted)) {
			i++;
			continue;	 
		} 
